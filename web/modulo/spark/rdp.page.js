...
function initServers(){
    ...
    var h = gw.value;
    if (!h){
        gw.value = hi5.browser.getHost() || "www.remotespark.com:8080";
    }
...
//     s += '&monitor=-1&monitorDef=' + encodeURIComponent('-1024:0:-1:767,0:0:1079:799');
    ...
    if (svGlobal.monitors && svGlobal.monitors.length){//multi monitor
		...
		var right = screen.width + left - 1;
		var bottom = screen.height + top - 1;
		var surface = new svGlobal.LocalInterface();
		surface.setMonitor(left, top, right, bottom);
